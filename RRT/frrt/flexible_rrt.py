...
class fRRT():
    ...
    def choose_parent(self, node_new, near_ind):
        ...
        if min_cost == float("inf"):
            ...
            # considering dist from parent and current node + parent cost + current node's trans cost
            ...
            # neglect distance between parent and current node
            # node_new.cost = min_cost + self.tran_weight * self.trans_cost(node_new)

            # change cost, only consider transition probability cost (does not work)
