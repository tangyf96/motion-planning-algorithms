...
class RRT():
    ...
    def GrowTree(self):
        ...
        while True:
            ...
            node_nearest_index, _ = self.FindNearestNode(
                self.nodelist, node_rnd, mode="Euclidean")
            node_nearest = self.nodelist[node_nearest_index]

            # Expand Tree
            theta = math.atan2(
                node_rnd[1] - node_nearest.y, node_rnd[0] - node_nearest.x)
            ...
            goal_dist = math.sqrt(
                (node_new.x - self.cur_goal.x)**2 + (node_new.y - self.cur_goal.y)**2)
    ...
    def FindNearestNode(self, nodelist, node_rnd, mode="Euclidean"):
        ...
        node_index = 0
        if mode == "Euclidean":
            min_dist = (self.start.x -
                        node_rnd[0]) ** 2 + (self.start.y - node_rnd[1]) ** 2
            for iter in range(len(nodelist)):
                if (nodelist[iter].flag == True):
                    distance = (nodelist[iter].x - node_rnd[0]) ** 2 + \
                        (nodelist[iter].y - node_rnd[1]) ** 2
            ...
            min_dist = (self.start.x -
                        node_rnd[0]) + (self.start.y - node_rnd[1])
            for iter in range(len(nodelist)):
                if (nodelist[iter].flag == True):
                    distance = abs(
                        nodelist[iter].x - node_rnd[0]) + abs(nodelist[iter].y - node_rnd[1])
    ...
    def CollisionCheck(self, node_new, new_obstacle=False, regrow=False):
        ...
        intercept = (node_new.x * node_parent.y - node_new.y *
                     node_parent.x) / (node_new.x - node_parent.x)
        ...
        for (x, y, radius) in obstacle:
            ...
            cross_flag = ((node_new.x - x) * (node_parent.x - x)
                          ) <= 0 or ((node_new.y - y) * (node_parent.y - y)) <= 0
            # dist2 is the distance from center of obstacle to the edge linking parent and child
            dist2 = abs(gradient * x - y + intercept) / \
                math.sqrt(gradient**2 + 1)
            if dist1 <= radius:
...
class Replan_RRT(RRT):
    ...
    def __init__(self, start, goal, stepsize, obstacle, TreeArea, old_path_index, new_obstacle, SampleRate=2, maxIter=500, nodelist=[]):
        ...
        """
        super(Replan_RRT, self).__init__(start, goal, stepsize,
                                         obstacle, TreeArea, SampleRate=2, maxIter=500)
    ...
    def regrow(self):
        ...
        index, _ = self.FindNearestNode(
            self.nodelist, [self.cur_goal.x, self.cur_goal.y], mode="Euclidean")
        ...
        while True:
            ...
            node_nearest_index, _ = self.FindNearestNode(
                self.replan_nodelist, node_rnd, mode="Euclidean")
            ...
            node_nearest_index, min_dist = self.FindNearestNode(
                self.nodelist, [node_new.x, node_new.y])
...
#                print(self.nodelist[node_nearest_index].parent)
                if not self.CollisionCheck(node_nearest, regrow=True):
                    ...
                    self.replan_nodelist.append(
                        self.nodelist[node_nearest_index])
                    return True
            if (step >= self.maxIter):
